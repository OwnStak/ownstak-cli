...
export const astroFrameworkAdapter: FrameworkAdapter = {
    ...
    async build(config: Config): Promise<void> {
        ...
        if (adapterName && adapterName !== '@astrojs/node') {
            ...
            logger.error(`You can do this by running: npx astro add node`);
            logger.error(`See more at: https://docs.astro.build/en/guides/integrations-guide/node/`);
            process.exit(1);
        }
...
async function loadAstroConfig() {
    ...
    if (!astroConfigPath) {
        throw new Error('Astro config file was not found. Please create an astro.config.mjs file.');
    }
