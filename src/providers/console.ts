...
import { ASSETS_DIR, COMPUTE_DIR, CONSOLE_API_URL, CONSOLE_URL, DEBUG_DIR, NAME, PERMANENT_ASSETS_DIR } from '../constants.js';
...
export default class ConsoleProvider extends Provider {
    ...
    async init() {
        ...
        const apiConfig = await ensureAuthenticated(this.options);

        const api = new ConsoleClient(apiConfig);

        // Use org, project and environment from options and config if provided.
        const initialOrganizationSlug = (this.options.organization || config.organization)?.toLowerCase();
        const initialProjectSlug = (this.options.project || config.project)?.toLowerCase();
        const initialEnvironmentSlug = (this.options.environment || config.environment || Config.getDefaultEnvironment())?.toLowerCase();
    ...
    async deploy() {
        ...
        const { api, environmentId, environmentSlug } = this.initResult;
        const { config } = this;

        let deploymentStatus = 'draft';
        const draftDeployment = await api.createDeployment(environmentId, {
